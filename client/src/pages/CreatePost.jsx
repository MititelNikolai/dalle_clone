...
const CreatePost = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      setLoading(true);
      try {
        const response = await fetch("http://192.168.100.35:8080/api/v1/post", {
          method: "POST",
          headers: {
    ...
    } else alert("Please enter a prompt and generate image");
  };
    ...
    if (form.prompt) {
      try {
        ...
        const response = await fetch(
          "http://192.168.100.35:8080/api/v1/dalle",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
